...
const fresh = () => ({ xp: 0, streak: { last: null, count: 0 }, badges: {}, met: {}, best: {}, fits: 0, renders: 0, mission: null, missionsDone: 0, daily: null, log: [], history: [] });
...
export function dailyBrief() {
  ...
  const pool = STYLISTS.filter(s => (s.unlock || 1) === 1);
  const s = pool[h % pool.length], o = OCCASIONS[(h >> 4) % OCCASIONS.length];
  return { date: t, stylist: s.id, occasion: o.id, target: 66 + (h >> 8) % 8, done: G.daily === t };
}
...
export function recordVerdict({ stylistId, occasion, total, photoKey, stylistsOnPhoto }) {
  ...
  G.history = [{ t: Date.now(), stylistId, occasion, total }, ...G.history].slice(0, 30);
  save();
  badge('first-fit');
...
export const bestEver = () => G.history.reduce((m, h) => Math.max(m, h.total), 0);
