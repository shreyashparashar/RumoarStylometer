...
export function recordVerdict({ stylistId, occasion, total, photoKey, stylistsOnPhoto }) {
  ...
  touchStreak();
  G.fits++;
  if (!G.met[stylistId]) { G.met[stylistId] = Date.now(); push(25, `Met ${byId[stylistId].name}`); }
