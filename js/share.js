...
export async function stylistCard({ photo, productImg, stylist, before, after, quote, occasion }) {
  ...
  x.font = `700 36px ${F}`; x.fillText(`— ${stylist.name}, ${stylist.role}`, 80, 1700);
  x.font = `500 26px ${F}`; x.globalAlpha = .7; x.fillText('Scores the look, never the man. Stylists are fictional characters.', 80, 1820); x.globalAlpha = 1;
