...
const avatar = (s, cls = 'av') => s.photo
  ? `<span class="${cls}" style="background:${s.pal.card}"><img src="${s.photo}" alt=""></span>`
...
async function analyse() {
  ...
  p.key = S.fingerprint(p.canvas); p.url = AI.toDataURL(p.canvas, 768); p.small = AI.toDataURL(p.canvas, 512);
...
function cardHTML(s) {
  ...
  return `<button class="scard ${u ? '' : 'locked'}" data-s="${s.id}" aria-pressed="${st.stylist === s.id}" style="--c1:${P.bg};--c2:${P.ink};--c3:${P.acc};--c4:${P.card}" ${u ? '' : `aria-label="${esc(s.name)}, locked until level ${s.unlock}"`}>
    <div class="art" style="background:${pattern(s.pattern, P.acc, P.bg)}">${s.photo ? `<img class="face" src="${s.photo}" alt="">` : `<span class="mono">${initials(s.name)}</span>`}
...
function stylist() {
  ...
  main.innerHTML = `<div class="kick"><b>4</b>The stylist</div><h2 class="h rise">${words('Pick your stylist.')}</h2>
    ...
    <div class="bar-foot"><button class="cta" id="next" ${sel ? '' : 'disabled'}>${sel ? `Get ${esc(sel.name)}'s verdict` : 'Choose a stylist'} <span class="arr">→</span></button></div>`;
...
async function verdict() {
  ...
  if (!AI.connected() || !st.seen) {
    main.innerHTML = `<div class="kick"><b>5</b>The verdict</div><h2 class="h">${words(AI.connected() ? 'The AI has not read your photo yet.' : 'The AI brain is not connected.')}</h2>
      <p class="lede">${esc(s.name)} only talks about what the AI actually sees in your photo. No guessing, no made-up verdicts.</p>${AI.connected() ? '' : setupCard()}<div class="row" style="margin-top:20px"><button class="ghost" id="back">Back to the scan</button></div>`;
    $('#back').onclick = () => go('scan'); return;
  }
  let li = 0;
  main.innerHTML = `<section class="thinking">${avatar(s, 'av xl')}<h2 class="h" id="thinkT">${esc(s.loading[0])}</h2><div class="dots"><i></i><i></i><i></i></div><p class="lede">Llama 4 Scout is looking at your photo through ${esc(first(s))}'s eyes.</p></section>`;
  ...
  try {
    r = await AI.verdict({ image: st.photo.url, seen: st.seen, occasion: st.occasion, vibes: vibeNames(), catalog: catalog(),
      stylist: { name: s.name, role: s.role, voice: s.voice, lane: s.lane, moves: s.moves, favourites: s.favourites },
...
function showVerdict(s, r) {
  ...
  <section class="verdict">
    <div class="vleft">
      ...
      <div class="stick match"><div>${r.vibe_match}%<small>${F1} match</small></div></div>
      <div class="rewards" id="rewards"></div>
    </div>
    <div>
      <div class="who">${avatar(s)}<div><b>${esc(s.name)}</b><span>${esc(s.role)}</span></div><span class="props-row">${s.props.map(p => `<i>${prop(p, s.pal.acc, s.pal.card)}</i>`).join('')}</span></div>
      <p class="speech" id="speech" aria-live="polite"><span class="caret"></span></p>
      <div class="aibadge">Seen and written by Llama 4 Scout on Cloudflare · face blurred before upload</div>
      <h3 class="h3">${F1}'s three moves</h3>
      <ol class="moves">${r.moves.map((m, i) => `<li style="--i:${i}">${esc(m)}</li>`).join('')}</ol>
      <div class="mission"><div><span class="lbl">Mission from ${F1} · +120 XP</span><b>${esc(r.mission.title)}</b><p>${esc(r.mission.task)} Re-shoot, come back to ${F1}, and beat ${r.total + 3}.</p></div><button class="ghost" id="accept" ${accepted ? 'disabled' : ''}>${accepted ? 'Accepted ✓' : 'Accept mission'}</button></div>
      <h3 class="h3">${F1}'s RUMOAR picks</h3>
      ...
      <div class="row" style="margin-top:28px"><button class="cta" id="restyle">Let ${F1} restyle me <span class="arr">→</span></button><button class="ghost" id="share">Share card</button><button class="ghost" id="restart">New fit</button></div>
      <div class="swap"><h3 class="h3">Ask someone else · same score, new advice</h3><div class="row">${others.map(o => { const u = GM.unlocked(o); return `<button data-s="${o.id}" class="${u ? '' : 'locked'}" ${u ? '' : `title="Unlocks at level ${o.unlock}"`}>${u ? avatar(o, 'mav') : `<span class="mav lockav">${o.unlock}</span>`}${u ? esc(o.name) : 'Locked'}</button>`; }).join('')}</div></div>
    </div>
  </section>`;
  ...
  $('#accept').onclick = e => { GM.acceptMission({ stylistId: s.id, title: r.mission.title, task: r.mission.task, base: r.total, photoKey: st.photo.key }); e.currentTarget.textContent = 'Accepted ✓'; e.currentTarget.disabled = true; blip(880, .1); toast(`Mission accepted. Re-shoot, then come back to ${first(s)}.`); };
...
async function studio(mode, s, r, product, gain = 0) {
  ...
  box.innerHTML = `<div class="stu"><div class="ba" id="ba"><div class="layer"><canvas id="baA" class="fill"></canvas></div><div class="layer before" id="baB"></div><div class="handle" id="baH"></div><input type="range" min="2" max="98" value="98" id="baR" aria-label="Before and after"><span class="pill l">Before</span><span class="pill r">AI render</span>
      <div class="busy" id="busy"><div class="dots"><i></i><i></i><i></i></div><b>${mode === 'tryon' ? `Rendering you with ${esc(product.name.replace('The ', ''))}…` : `${esc(first(s))} is restyling you…`}</b><span>FLUX.2 on Cloudflare · usually 10–30 s</span></div></div>
    <div class="stuinfo"><span class="lbl">${mode === 'tryon' ? 'AI try-on' : 'AI restyle'}</span><b>${mode === 'tryon' ? esc(product.name) : `${esc(s.name)}'s version of you`}</b><p>${mode === 'tryon' ? 'Your photo plus this RUMOAR piece, rendered by AI. Your face stays blurred.' : esc(r.restyle_prompt)}</p><div id="stuAct"></div></div></div>`;
  ...
  try {
    ...
    if (mode === 'tryon') body.productImage = await AI.urlToDataURL(product.img, 512);
...
history.replaceState({ step: start }, '', '#' + start); st.step = start; render();
