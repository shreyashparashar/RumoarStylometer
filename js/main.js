...
const avatar = (s, cls = 'av') => s.photo
  ? `<span class="${cls}" style="background:${s.pal.card};color:${s.pal.bg}"><img src="${s.photo}" alt="${esc(s.name)}" referrerpolicy="no-referrer" onerror="this.replaceWith('${initials(s.name)}')"></span>`
...
// ------------------------------------------------------------------ theme + stylist world
const BASE = { bg: '#2448FF', ink: '#FFFFFF', acc: '#FFE14A', card: '#0A0A0A' };
...
// ------------------------------------------------------------------ HOME
// The score scale, shared by the drop card and the "how it's scored" panel. Matches the Worker's calibration.
...
function home() {
  ...
  <section class="hero">
    ...
    <label class="dropcard" id="drop" for="file" tabindex="0" aria-label="Drop a photo here or press Enter to choose one">
      <span class="burst"><b>${last ? last.total : '??'}</b><small>${last ? esc(bandOf(last.total)) : 'out of 100'}</small></span>
      <span class="dz"><i class="dzicon">${prop('camera', '#FF5A1F', '#0A0A0A')}</i><b>Drop your fit here</b><span>or tap to pick a photo. Head to shoes works best.</span></span>
      <span class="tags" aria-hidden="true"><em>Fit</em><em>Colour</em><em>Occasion</em><em>Cohesion</em><em>Finishing</em></span>
      ${last ? `<span class="lastline">Last fit: ${esc(occName(last.occasion))}, ${esc(first(byId[last.stylistId]))} vibe, ${ago(last.t)}</span>` : ''}
    </label>
  </section>
  ...
  <section class="roster">
    ...
    <div class="rail">${STYLISTS.map(x => { const u = GM.unlocked(x); return `<button class="rcard ${u ? '' : 'locked'}" data-pick="${x.id}" aria-pressed="${st.stylist === x.id}" style="--c:${x.pal.acc};--cb:${x.pal.bg};--ci:${x.pal.ink}">
  ...
  <section class="rubric">
    ...
    <div class="pgrid">${S.PILLARS.map(([k, n]) => `<div class="pc-${k}"><b>20</b><h3>${esc(n)}</h3><p>${pillarsCopy[k]}</p></div>`).join('')}</div>
    <div class="twocol">
...
// ------------------------------------------------------------------ OCCASION
const OCC_LOOK = { college: ['#FFE14A', 'cap'], 'date-night': ['#FF5A1F', 'rose'], office: ['#FFFFFF', 'watch'], interview: ['#3FC7FF', 'envelope'], wedding: ['#FFB23F', 'crown'],
  party: ['#B08CFF', 'boombox'], concert: ['#B8F23A', 'mic'], festival: ['#FF7A59', 'sparkle'], travel: ['#7FE0C8', 'passport'] };
function occasion() {
  ...
  <div class="occ-grid">${OCCASIONS.map((o, i) => `<button class="tile" data-o="${o.id}" aria-pressed="${st.occasion === o.id}" style="--tc:${OCC_LOOK[o.id]?.[0] || '#fff'};--tr:${[-1.2, .8, -.6, 1, -1, .6, -.8, 1.2, -.4][i % 9]}deg">
      <i class="ticon">${prop(OCC_LOOK[o.id]?.[1] || 'star', '#0A0A0A', '#FFFFFF')}</i><span class="code">${esc(o.code)}</span><b>${esc(o.name)}</b><p>${esc(o.line)}</p>
...
function showVerdict(s, r) {
  ...
  setTimeout(() => {
    ...
    if (r.total >= 65) confetti([s.pal.acc, s.pal.bg, s.pal.ink, s.pal.card]); chord(440 + (r.total - 50) * 6);
  }, 1500);
}
function slot(el, to) {
