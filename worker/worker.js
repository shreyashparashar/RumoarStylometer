...
const OCCASIONS = { 'date-night': 'a date night dinner', concert: 'a concert', office: 'the office', college: 'college', festival: 'a music festival',
  interview: 'a job interview', wedding: 'an Indian wedding function', party: 'a party or club night', travel: 'a travel day' };
...
export default {
  async fetch(req, env) {
    ...
    // Health check is always open, so you can visit it straight from the address bar.
...
const SEE_SCHEMA = {
  ...
  properties: {
    ...
    condition_issues: { type: 'array', items: { type: 'string' }, description: 'visible wrinkles, creases, stains, dirt, fading, fraying, holes (unless clearly a deliberate design), pilling, stretched collars, scuffed or dirty shoes' },
    fit_issues: { type: 'array', items: { type: 'string' }, description: 'garment fit problems: sleeves too long, trousers pooling or too short, shirt too tight or billowing, shoulder seams dropping unintentionally, gaping buttons' },
    uniform: { type: 'string', description: 'if this is a work or service uniform (waiter, security, delivery, school, factory, hospital), name it; otherwise "none"' },
    effort: { type: 'string', enum: ['none', 'low', 'moderate', 'high', 'very high'], description: 'how much deliberate styling intent is visible' },
    polish: { type: 'integer', description: '1-10 overall garment polish: 1-2 damaged or dirty, 3-4 worn-out or sloppy, 5 plain and clean, 6 neat everyday, 7 well put together, 8 sharp, 9-10 editorial' },
  },
  required: ['is_outfit_photo', 'framing', 'garments', 'accessories', 'palette', 'style_read', 'condition_issues', 'fit_issues', 'uniform', 'effort', 'polish'],
};
async function see(env, { image }) {
  ...
  const system = 'You are a precise fashion cataloguer for a styling app. List ONLY what is clearly visible. ' +
    ...
    'Be a strict, honest inspector, not a cheerleader. List every visible condition problem and fit problem. ' +
    'Work and service uniforms, gym wear, sleepwear and worn-out everyday clothes are not styled outfits: say so in uniform/effort. ' +
    'polish and effort describe the CLOTHES only. Most everyday photos are polish 5-6 and effort low or moderate; 8+ is rare.';
...
async function verdict(env, { image, seen, occasion, occasionInfo, vibes = [], stylist, catalog = [], fixedPillars }) {
  if (!seen || !stylist) throw new Error('seen and stylist required');
  const occ = OCCASIONS[occasion] || String(occasion || 'an everyday outing').replace(/^custom:/, '').slice(0, 80);
  const occJudged = occasionInfo?.judged?.length ? ` For ${occ}, weigh most: ${occasionInfo.judged.join('; ')}.` : '';
  ...
  const pillarRule = fixedPillars
    ...
    : `PILLARS: score each 0-20 as a strict, neutral judge FIRST, identical for every vibe; the vibe changes the words, never the numbers.
Anchors for EVERY pillar: 4 = broken or wrong, 8 = poor, 11 = plain/average, 13 = decent, 15 = good, 17 = excellent, 19-20 = exceptional (rare).
fit = how each garment fits and drapes (sleeve and trouser length, shoulder seams, tightness). colour = how the palette works together.
occasion = right for ${occ}.${occJudged} cohesion = do the pieces tell one intentional story.
finishing = visible bag, watch, eyewear, belt, jewellery, shoe condition, tuck/cuff details. With no accessories visible, finishing is 7-11.
TOTAL calibration: 20-40 sloppy, dirty, damaged or badly fitting; 40-55 plain functional clothes with no styling intent (a work uniform, old tee and trackpants, a rumpled shirt); 55-65 clean but basic; 65-75 clearly styled and well fitted; 75-85 sharp and right for the occasion; 85+ editorial, almost never.
Wrinkles, stains, fraying, faded or worn-out pieces, scuffed shoes and bad fit must pull scores down hard. Do not reward a photo for being clear or a person for being confident; score the clothes. Each "why" = one sentence naming a specific item.`;
  const system =
...
9. If the photo catalogue lists condition_issues, fit_issues or a uniform, the verdict must name them plainly and at least one move must fix one. Honest beats nice.
${pillarRule}
...
restyle_prompt = one sentence for an image model describing the restyled outfit in this vibe: every garment with colour and fabric, footwear, accessories, how worn. NO person or celebrity names, no face or body words.`;
  ...
  if (!fixedPillars) r.caps = calibrate(r.pillars, seen, occasion);
...
// ------------------------------------------------------------------ strict calibration
// Hard ceilings from what the inspector saw, applied after the model scores. The model tends to be
// generous; these rules make sure worn-out, uniform or unstyled clothes can never land in the 70s.
// Returns the list of caps applied so the UI can say why a score is held down.
function calibrate(p, seen = {}, occasion) {
  const caps = [];
  const cap = (k, max, why) => { if (p[k].score > max) { p[k].score = max; if (!caps.includes(why)) caps.push(why); } };
  const all = (max, why) => ['fit', 'colour', 'occasion', 'cohesion', 'finishing'].forEach(k => cap(k, max, why));
  const cond = (seen.condition_issues || []).filter(Boolean), fitI = (seen.fit_issues || []).filter(Boolean);
  const uniform = seen.uniform && !/^(none|no|n\/a|)$/i.test(String(seen.uniform).trim());
  const effort = seen.effort || 'moderate', polish = Number(seen.polish) || 6;
  const acc = (seen.accessories || []).length, shoes = seen.footwear && !/not visible/i.test(seen.footwear);

  if (seen.is_outfit_photo === false) all(6, 'Not an outfit photo');
  if (cond.length) { cap('finishing', 10, 'Clothes in poor condition'); cap('cohesion', 13, 'Clothes in poor condition'); }
  if (cond.length >= 2) { cap('fit', 11, 'Clothes in poor condition'); all(12, 'Clothes in poor condition'); }
  if (fitI.length) cap('fit', Math.max(6, 14 - 2 * (fitI.length - 1)), 'Fit problems');
  if (uniform) { cap('occasion', occasion === 'office' ? 11 : 7, 'Work uniform, not a styled outfit'); cap('cohesion', 11, 'Work uniform, not a styled outfit'); cap('colour', 12, 'Work uniform, not a styled outfit'); }
  if (effort === 'none') { all(10, 'No styling effort visible'); }
  else if (effort === 'low') { cap('cohesion', 11, 'Little styling effort'); cap('finishing', 10, 'Little styling effort'); cap('colour', 13, 'Little styling effort'); }
  if (!acc && !shoes) cap('finishing', 10, 'No accessories or shoes visible');
  else if (!acc) cap('finishing', 12, 'No accessories');
  if (seen.framing === 'upper body') cap('fit', 14, 'Only half the outfit is visible');

  // overall ceiling from polish: 5 -> 60, 6 -> 66, 7 -> 72, 8 -> 78, 9 -> 84, 10 -> 90
  const ceiling = Math.round(30 + Math.max(1, Math.min(10, polish)) * 6);
  const total = () => Object.values(p).reduce((a, x) => a + x.score, 0);
  if (total() > ceiling) {
    const k = ceiling / total();
    for (const key of Object.keys(p)) p[key].score = Math.floor(p[key].score * k);
    caps.push(`Overall polish ${polish}/10 caps the score at ${ceiling}`);
  }
  return caps;
}
